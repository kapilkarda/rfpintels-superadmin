...
dispatch({ type: types.PUT_ADDUSERBTN_PENDING });
    const res = await axios.put(`http://rfpintels-services.eastus.cloudapp.azure.com/userservices/user/updateAddUser`,data,
    {
    headers: {
    Authorization: `Bearer ${tokens}`,
    },
...
export const CompanyDetailsAdded = (data) => async (dispatch) => {
     const tokens = localStorage.getItem("token");
        ...
        console.log(data , "DATA IN REDUX ACTION AFTER API RES")

 
        const res = await axios.put(
        `http://rfpintels-services.eastus.cloudapp.azure.com/userservices/user/editCompanyProfile`,data,
        {
        headers: {
...
dispatch({ type: types.GET_USER_DATA_EDIT_PENDING });
const res = await axios.put(
`http://rfpintels-services.eastus.cloudapp.azure.com/userservices/user/updateAddUser`,data,
{
headers: {
Authorization: `Bearer ${tokens}`,
},
